...
### Fetch the secrets from Vault using userpass auth
...
this function will use the `vault` module to fetch the secrets and for testing will
...
You are probably asking why we are passing the vault username and password as secrets
the whole purpose being that we did not need to share secrets.

This is so that we can test things locally, in CI/CD we are going to authenticate Vault
using an OIDC token that is provided by the CI/CD provider.

But before we get to that, let's modify the code to use the secrets from Vault.
...
Let's run the pipeline locally to see if it works.
...
## Modify pipeline to use OIDC auth

Let's now modify the pipeline to use the secrets from Vault using OIDC auth for GitHub Actions.
We can just copy the code from the `fetchDeploymentSecretUserpass` function and modify it
to use the `jwt` auth method. In the real world we would refact the code to get rid of the 
duplication, but in the real world I would also no be doing this live.
...
func (d *Build) fetchDeploymentSecretOIDC(ctx context.Context, vaultHost, vaultNamespace string, jwt *Secret, jwtAuthPath string) (*VaultSecrets, error) {
	...
	vc := cli.Vault().
		WithHost(vaultHost).
		WithNamespace(vaultNamespace).
		WithJwtauth(jwt, jwtAuthPath)
```

In GitHub Actions the OIDC token is not automatically provided, we need to fetch it from the
GitHub OIDC service. We can do this by making a call to the GitHub API. Let's write a function
to do this.
...
Finally we can modify our all function to use this new function to get the JWT token and then
fetch the secrets from Vault. First we change the signature to use optional parameters.
...
Then we can handle which auth method we are going to use.

```go
var secrets *VaultSecrets

if vaultUsername != "" && vaultPassword != nil {
	secrets, err = b.fetchDeploymentSecretUserpass(ctx, vaultAddr, vaultUsername, vaultPassword, vaultNamespace)
	if err != nil {
		return fmt.Errorf("failed to fetch deployment secret:%w", err)
	}
}
...
And adding the new OIDC method.
...
Let's take a look at the completed code in GitHub.
